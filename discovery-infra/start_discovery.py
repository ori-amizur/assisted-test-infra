...
def nodes_flow(client, cluster_name, cluster):
    ...
    main_cidr = args.vm_network_cidr if is_ip4 else args.vm_network_cidr6
    secondary_cidr = machine_net.provisioning_cidr_v4 if is_ip4 else machine_net.provisioning_cidr_v6
    ...
    if client:
        ...
        if not (cluster_info.api_vip and cluster_info.ingress_vip):
            ...
            if args.master_count == 1:
                tf.change_variables({"single_node_ip": helper_cluster.Cluster.get_ip_for_single_node(client, cluster.id, main_cidr, ipv4_first=is_ip4)})
        ...
        if is_none_platform_mode() and args.master_count > 1:
            ...
            load_balancer_config_file = utils.render_load_balancer_config_file(load_balancer_ip, master_ips)
            tf.change_variables({"load_balancer_ip": load_balancer_ip, "load_balancer_config_file": load_balancer_config_file})
