...
start_load_balancer:
	id=`podman ps --quiet --filter "name=load_balancer"` ; ( test -z "$$id" && podman run -d --rm --net=host --name=load_balancer -v $(HOME)/.test-infra/etc/nginx/conf.d:/etc/nginx/conf.d load_balancer:latest ) || ! test -z "$$id"

stop_load_balancer:
	podman rm -f load_balancer && rm -f  $(HOME)/.test-infra/etc/nginx/conf.d:/etc/nginx/conf.d/* || /bin/true
